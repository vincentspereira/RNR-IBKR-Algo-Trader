...
class UniverseMembership:
    ...
    def is_member_on(self, as_of: date) -> bool:
        if as_of < self.added:
            return False
        if self.removed is not None and as_of >= self.removed:
            return False
        return True
...
class Universe:
    ...
    def members_on(self, as_of: date) -> tuple[str, ...]:
        ...
        return tuple(
            sorted(m.symbol for m in self.memberships if m.is_member_on(as_of))
        )
...
def get_universe(name: str) -> Universe:
    ...
    if key not in BUILTIN_UNIVERSES:
        raise KeyError(
            f"Unknown universe {name!r}. Known: {sorted(BUILTIN_UNIVERSES)}"
        )
