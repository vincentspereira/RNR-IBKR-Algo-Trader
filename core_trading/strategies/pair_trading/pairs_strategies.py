...
from .pair_selection import PairMetrics, PairSelector
...
#                 logger.warning("Invalid or empty price data provided")
#                 return []

#             if len(data1) != len(data2):""
#                 logger.warning("Price series have different lengths")
#                 min_len = min(len(data1), len(data2))
#                 data1 = data1.iloc[-min_len:]
#                 data2 = data2.iloc[-min_len:]

            # Ensure we have enough data for analysis
#             if len(data1) < self.config.min_data_points:
# logger.debug("
#                     f"Insufficient data points: {len(data1)} < {self.config.min_data_points}"
# )
#                 return []
...
#             spread_data = self._calculate_spread_metrics(data1, data2)
#             if spread_data is None:
#                 return []

            # Calculate cointegration metrics
#             cointegration_metrics = self._calculate_cointegration_metrics(data1, data2)

            # Generate entry signals based on z-score thresholds"
#             current_z_score = spread_data["z_score"].iloc[-1]
# previous_z_score = ("
# spread_data["z_score"].iloc[-2]"
#                 if len(spread_data["z_score"]) > 1
# else 0
# )
...
#             if (
#                 current_z_score > entry_threshold
# and previous_z_score <= entry_threshold
# ):
#                 signal_type = SignalType.SELL  # Short the spread
#                 signal_strength = self._calculate_signal_strength(abs(current_z_score))
#                 confidence = min(0.95, abs(current_z_score) / (entry_threshold * 2))
...
# and previous_z_score >= -entry_threshold
# ):
#                 signal_type = SignalType.BUY  # Long the spread
#                 signal_strength = self._calculate_signal_strength(abs(current_z_score))
#                 confidence = min(0.95, abs(current_z_score) / (entry_threshold * 2))

            # Exit signal (mean reversion)
#             elif (
#                 abs(current_z_score) < exit_threshold
# and abs(previous_z_score) >= exit_threshold
# ):
...
#                         symbol=f"{self.config.symbol1}_{self.config.symbol2}",
#                         price=float(data1.iloc[-1]),  # Price of first asset
#                         volume=int(volume1.iloc[-1]) if volume1 is not None else 0,
# metadata={
# "pair_id": f"{self.config.symbol1}_{self.config.symbol2}","
# "z_score": float(current_z_score),"
# "spread": float(spread_data["spread"].iloc[-1]),"
# "hedge_ratio": float("
#                                 cointegration_metrics.get("hedge_ratio", 1.0)
# ),"
# "price_asset1": float(data1.iloc[-1]),"
# "price_asset2": float(data2.iloc[-1]),"
# "cointegration_pvalue": float("
#                                 cointegration_metrics.get("p_value", 1.0)
# ),"
# "half_life": float("
#                                 cointegration_metrics.get("half_life", 10.0)
# ),
# },
# )
...
#             logger.error(f"Error generating pair trading signals: {str(e)}")
#             return []

#     @abstractmethod
#     def calculate_position_size(
# self, signal: TradingSignal, portfolio_value: float, current_exposure: float
# ) -> Tuple[float, float]:"
#         "Calculate position sizes for both assets."
#         try:
            # Input validation"
#             if signal is None or portfolio_value <= 0:""
#                 logger.warning("Invalid signal or portfolio value")
#                 return (0.0, 0.0)
...
# price_asset1 = metadata.get("price_asset1", signal.price)"
#             price_asset2 = metadata.get("price_asset2", signal.price)
...
#             z_score_multiplier = min(1.5, 1.0 + abs(z_score) / 10.0)

            # Check exposure limits
#             max_exposure = self.config.max_portfolio_exposure
#             if current_exposure + base_position_pct > max_exposure:
#                 base_position_pct = max(0, max_exposure - current_exposure)
...
#             position_value = portfolio_value * adjusted_position_pct
            ...
            # Calculate quantities for each asset
#             if (
#                 signal.signal_type == SignalType.BUY
# ):  # Long spread (long asset1, short asset2)
#                 quantity_asset1 = position_value / price_asset1
# quantity_asset2 = (
#                     -(quantity_asset1 * hedge_ratio * price_asset1) / price_asset2
# )
...
#                 quantity_asset1 = -position_value / price_asset1
# quantity_asset2 = (
#                     abs(quantity_asset1) * hedge_ratio * price_asset1
# ) / price_asset2
...
#             if abs(quantity_asset1 * price_asset1) < min_position_value:
#                 if quantity_asset1 != 0:
# quantity_asset1 = (
#                         min_position_value
# / price_asset1
#                         * (1 if quantity_asset1 > 0 else -1)
# )
# quantity_asset2 = (
#                         -(quantity_asset1 * hedge_ratio * price_asset1) / price_asset2
# )
...
# lot_size_1 = getattr(self.config, "lot_size_asset1", 1.0)"
#             lot_size_2 = getattr(self.config, "lot_size_asset2", 1.0)

#             quantity_asset1 = round(quantity_asset1 / lot_size_1) * lot_size_1
#             quantity_asset2 = round(quantity_asset2 / lot_size_2) * lot_size_2

# logger.debug("
#                 f"Position sizes calculated: Asset1={quantity_asset1}, Asset2={quantity_asset2}"
# )

#             return (float(quantity_asset1), float(quantity_asset2))
...
# ) -> Optional[Dict]:"
#         "Calculate spread and z-score metrics."
#         try:
            # Calculate log prices for better stationarity
#             log_data1 = np.log(data1)
#             log_data2 = np.log(data2)

            # Calculate spread (assuming hedge ratio of 1 for simplicity)
#             spread = log_data1 - log_data2

            # Calculate rolling statistics
#             window = min(30, len(spread) // 2)  # Adaptive window size
...
#     def _calculate_cointegration_metrics(
# self, data1: pd.Series, data2: pd.Series
# ) -> Dict:"
#         "Calculate cointegration metrics including hedge ratio."
#         try:
...
#             hedge_ratio = model.coef_[0]
            ...
            # Calculate residuals for cointegration test
#             residuals = y - model.predict(X)

            # Simple half-life calculation
            # Using AR(1) model: residuals[t] = alpha * residuals[t-1] + error
#             lagged_residuals = residuals[:-1]
#             current_residuals = residuals[1:]

#             if len(lagged_residuals) > 0:
#                 alpha = np.corrcoef(lagged_residuals, current_residuals)[0, 1]
# half_life = (
#                     -np.log(2) / np.log(abs(alpha))
#                     if abs(alpha) > 0 and abs(alpha) < 1
# else 10.0
# )
#             else:
#                 half_life = 10.0

            # Simplified p-value (would need proper ADF test for accuracy)
#             p_value = 0.05 if abs(alpha) > 0.1 else 0.1
...
#         except Exception as e:""
# logger.error(f"Error calculating cointegration metrics: {str(e)}")"
#             return {"hedge_ratio": 1.0, "half_life": 10.0, "p_value": 0.1, "alpha": 0.0}

#     def _calculate_signal_strength(self, z_score_magnitude: float):
#         "Calculate signal strength based on z-score magnitude."
#         try:
#             if z_score_magnitude >= 3.0:
#                 return SignalStrength.EXTREME
#             elif z_score_magnitude >= 2.5:
#                 return SignalStrength.STRONG
#             elif z_score_magnitude >= 2.0:
...
# self, volume1: pd.Series, volume2: pd.Series
# ) -> bool:"
#         "Validate volume confirmation for signal."
#         try:
            # Check if recent volume is above average
#             window = min(20, len(volume1) // 2)

#             avg_volume1 = volume1.rolling(window=window).mean().iloc[-1]
#             avg_volume2 = volume2.rolling(window=window).mean().iloc[-1]

#             current_volume1 = volume1.iloc[-1]
#             current_volume2 = volume2.iloc[-1]

            # Volume should be at least 80% of average
#             volume_threshold = 0.8

#             return (
#                 current_volume1 >= avg_volume1 * volume_threshold
# and current_volume2 >= avg_volume2 * volume_threshold
# )

#         except Exception as e:""
#             logger.error(f"Error validating volume confirmation: {str(e)}")
...
# corr_result = self.correlation_analyzer.calculate_correlation(
#                 series1, series2
# )
# "
            # Cointegration test
# coint_result = self.cointegration_tester.engle_granger_test(
#                 series1, series2
# )
# "
            # Spread analysis
...
# is_valid = (
#                 abs(corr_result.correlation) >= self.config.min_correlation
# and abs(corr_result.correlation) <= self.config.max_correlation
# and corr_result.pvalue <= 0.05
# and coint_result.is_cointegrated
# and coint_result.pvalue <= self.config.cointegration_pvalue
# and spread_result.stationarity_result.is_stationary
# and self.config.min_half_life
...
#                 if (
#                     position.symbol1 in current_prices
# and position.symbol2 in current_prices
# ):
...
# "
# current_zscore = ("
#                 indicators["zscore"].iloc[-1] if len(indicators["zscore"]) > 0 else 0
# )
# current_regime = ("
# indicators["regime"].iloc[-1]"
#                 if len(indicators["regime"]) > 0
# else RegimeType.UNKNOWN.value
# )
...
#                         should_exit = True
                ...
                # Stop loss condition
#                 if position.stop_loss and position.current_pnl <= -abs(
#                     position.stop_loss
# ):
#                     should_exit = True
...
# and position.current_pnl >= position.take_profit
# ):
#                     should_exit = True

                # Regime change condition
#                 if self.config.regime_filter and current_regime not in [
#                     RegimeType.MEAN_REVERTING.value,
#                     RegimeType.STABLE.value,
# ]:
#                     should_exit = True
...
# 1 for pos in self.active_positions.values() if pos.current_pnl > 0
# )
# "
# metrics = {"
# "total_pnl": total_pnl,"
...
# else 0,"
# "avg_pnl_per_position": total_pnl / total_positions
#                 if total_positions > 0
# else 0,"
# "max_favorable_excursion": max(
#                     (pos.max_favorable for pos in self.active_positions.values()),
#                     default=0,
# ),"
# "max_adverse_excursion": min(
#                     (pos.max_adverse for pos in self.active_positions.values()),
#                     default=0,
# ),
# }
...
# volume_combined = (
#                         volume_aligned.iloc[:, 0] + volume_aligned.iloc[:, 1]
# )

            # Calculate indicators
# indicators = self.indicator_suite.calculate_all_indicators(
#                 spread, volume_combined
# )
...
# else RegimeType.UNKNOWN.value
# )
# latest_bb_percent_b = indicators.get("
#                 "bollinger_percent_b", pd.Series([0.5])
# ).iloc[-1]"
# latest_squeeze = indicators.get("bollinger_squeeze", pd.Series([0])).iloc[
#                 -1
# ]

            # Only trade in mean-reverting or stable regimes
#             if self.config.regime_filter and latest_regime not in [
#                 RegimeType.MEAN_REVERTING.value,
#                 RegimeType.STABLE.value,
# ]:
#                 return signals
...
#                         latest_zscore, latest_regime, latest_squeeze
# )
...
#             confidence_multiplier = signal.confidence
            ...
            # Check exposure limits
#             if (
#                 current_exposure + base_position_pct
# > self.config.max_portfolio_exposure
# ):
# base_position_pct = max(
#                     0, self.config.max_portfolio_exposure - current_exposure
# )
...
#                     -quantity_1
#                     * signal.hedge_ratio
#                     * signal.entry_price_1
# / signal.entry_price_2
# )
#             else:
                # Short asset1, long asset2
#                 quantity_1 = -position_value / signal.entry_price_1
...
#     def _calculate_signal_strength(self, zscore_abs: float):
#         "Calculate signal strength based on Z-Score magnitude."
#         if zscore_abs >= 3.0:
#             return SignalStrength.EXTREME
#         elif zscore_abs >= 2.5:
...
# self, zscore: float, regime: str, squeeze: float
# ) -> float:"
...
#             if self.config.regime_filter and latest_regime != RegimeType.TRENDING.value:
#                 return signals
...
#                     latest_momentum, momentum_threshold
# )
# confidence = self._calculate_momentum_confidence(
#                     latest_momentum, latest_velocity
# )
...
# strength = self._calculate_momentum_strength(
#                     abs(latest_momentum), momentum_threshold
# )
# confidence = self._calculate_momentum_confidence(
#                     abs(latest_momentum), abs(latest_velocity)
# )
...
# self, momentum_abs: float, threshold: float
# ) -> SignalStrength:"
...
#     def _calculate_momentum_confidence(self, momentum: float, velocity: float):
