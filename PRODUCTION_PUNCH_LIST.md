...
2. **PostgreSQL order persistence.** [ADDRESSED 2026-06-02] `OrderStore` is now wired into the bootstrap: `main.py` builds it via `_build_order_store()` and passes it to `ExecutionEngine`, whose `initialize()` reconciles in-flight orders against the broker on startup. Persistence is **mandatory in live mode** (the bootstrap refuses to start without a reachable database) and optional-with-loud-warning in paper mode (so a no-DB smoke test can still run). Engaging it at runtime still requires a provisioned PostgreSQL instance and populated `POSTGRES_*` env vars; a connectivity probe decides on/off. Covered by `tests/unit/test_bootstrap_persistence.py`.
3. **Sector classification.** [ADDRESSED 2026-06-02] `_calculate_sector_exposures()` now computes dollar-weighted GICS sector exposure via a new injectable `SectorClassifier` (static reference map of ~105 liquid US equities; unknown symbols fall to an `Unclassified` bucket). This makes the monitoring-loop `sector_exposure` risk limit functional and fixes a latent bug: the old `{"Unclassified": 1.0}` stub *permanently* exceeded the 0.25 limit, so every non-empty portfolio falsely breached every cycle. Broader or point-in-time-correct coverage can be supplied by injecting a vendor/broker-backed classifier into `RiskEngine`. Covered by `tests/unit/test_sector_classifier.py`.
